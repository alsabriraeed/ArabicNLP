...
#                    ,'tfidf','pmi','okapibm25','tfrtf','cew','tfrstf',''] 
    ...
    versions =['version5.1']
#    versions =["version3.1"]
    models = ['pmilogWLDA','pmi']# pmilogWLDA #updatetf #'logWLDA'
#    models = ['cew']#,'cew'//,'tfrstf'
            ...
            # tfidf new for the paper "Improved TFIDF in big news retrieval: An empirical study"
                for mod in range(len(models)):
                    if (models[mod]=='improvedtfidf'):
                        ...
                        # tfidf    
                        weighted_matrix2 = weights.tf(numpy_matrix)
                    
                        weighted_array2 = weighted_matrix2.toarray()
                        total_weight3=np.asarray(weighted_array2).sum(axis=0)
                        
                        writingTotalweighttofile(total_weight3,path +  'tf.txt')
                        writingtofile( weighted_array2,path + 'singletf.txt')
                        
                    elif (models[mod]=='updatetf'):
                        # tfidf    
                        weighted_matrix2 = weights.updatetf(numpy_matrix)
                    
                        weighted_array2 = weighted_matrix2.toarray()
                        total_weight3=np.asarray(weighted_array2).sum(axis=0)
                        
                        writingTotalweighttofile(total_weight3,path +  'updatetf.txt')
                        writingtofile( weighted_array2,path + 'singleupdatetf.txt')
                        
                    elif (models[mod]=='pmilogWLDA'):
                        # tfidf    
                        weighted_matrix2 = weights.pmi_log_WLDA(numpy_matrix)
                        weighted_array2 = np.asarray(weighted_matrix2)
