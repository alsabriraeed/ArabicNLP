...
#    versions =["version5.1"]
    versions =["version2.1","version3.1","version4.1","version5.1","version6.1","version7.1"]
...
#    datasettype='long' # 'long'
    datasettypes = ['short','long']# ,'short', 'long'
    ...
    #            name =dataset
            ...
            # tfidf new for the paper "Improved TFIDF in big news retrieval: An empirical study"
                for mod in range(len(models)):
                    if (models[mod]=='improvedtfidf'):
                    ...
                    elif (models[mod]=='improvedhammingtfidf'):
                        
                        D = weights.hammingpairwise_distance(X)
                        D = np.asarray(D)
                        ...
                        writingTotalweighttofile(total_weight3,path +  'improvedhammingtfidf.txt')
                        writingtofile( weighted_matrix2,path + 'singleimprovedhammingtfidf.txt')
                        ...
                        # tfidf    
                        weighted_matrix2 = weights.logentropy(numpy_matrix)
                    
                        weighted_array2 = weighted_matrix2.toarray()
                        total_weight3=np.asarray(weighted_array2).sum(axis=0)
                        
                        writingTotalweighttofile(total_weight3,path +  'oldentropy.txt')
                        writingtofile( weighted_array2,path + 'singleoldentropy.txt')
...
#                        writingTotalweighttofile(logldaentropyweight,path+ 'CEW.txt')
                        ...
                        model = version.replace('.1','')
                        obj = TFRFST.main(path,name,version,model)
